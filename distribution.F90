...
!
! !REVISION HISTORY:
!  CVS:$Id: distribution.F90 28439 2011-05-18 21:40:58Z njn01 $
!  CVS:$Name:  $
...
!----------------------------------------------------------------------
   ...
   case default
      ...
      call exit_licom(sigAbort,'distribution: unknown distribution type')
...
!----------------------------------------------------------------------
   ...
   call POP_CommCreateCommunicator(dist%communicator, nprocs)
...
!----------------------------------------------------------------------
   ...
   if (minval(proc_tmp) < 1) then
      call exit_licom(sigAbort,'Load-balanced distribution failed')
   endif
...
!----------------------------------------------------------------------
   ...
   if (nprocs_x == 0) then
      call exit_licom(sigAbort,'Unable to find 2d processor config')
   endif
