...
!Any comments please send to huangwenyu@mail.tsinghua.edu.cn                      
    ...
    use precision_mod
    use constant_mod
    use pconst_mod, only : iday
    use utils
    implicit none

contains
    subroutine shengjin_calc(array,nmax,real_x,ii,jj)
        implicit none
        real(r8), parameter   :: epsln1 = 1.0d-6
...
!
        real_x = 1.0D0
        if (abs(sa) <= epsln1 .and. abs(sb) <= epsln1) then
            if ((-sd/sc)<0.0d0) then
...
!                   print *,  "Two real with one positive at least 2!"
        ...
        if (abs(A) <= epsln1 .and. abs(B) <= epsln1) then
            if ((-sc/sb) < 0.0d0) then
...
!               print *, y(1), y(2),sb, sa,real_x
        ...
        real_x = min ( real_x, 2.0D3)
        real_x = max ( real_x, 1.0D0)
    ...
    subroutine newton_iteration(array,nmax,out0)
        ...
        integer     :: counter, k, ii, jj
        real(r8)    :: x, fx, dfx
        logical     :: if_converge
        
        call best_initial_guess(array(:),nmax,start0)
        call shengjin_calc(array(:),nmax,x,ii,jj)
        out0 = x
        return
        !start0  =   10.0d0
1000    continue
    ...
    Subroutine prepare_pi(Ri,Rrho,out_pi)
        ...
        real(r8), parameter   :: pi0_1   =   1.0d0/(sqrt(27.0d0/5.0d0*Ko**3)*(1+1.0d0/sig_t))
        ...
        if (Ri > zero .and. Rrho > zero) then
            out_pi(1) = pi0_1/(1.0d0 + Ri*Rrho/(a + Rrho))
            out_pi(2) = pi0_2/(1.0d0 + Ri)*(1.0d0+2.0d0*Ri*Rrho/(1.0d0+Rrho*Rrho))
            out_pi(3) = pi0_3
            out_pi(4) = pi0_4/(1.0d0 + Ri/(1.0d0 + a*Rrho))
            out_pi(5) = pi0_5
        end if

        if (Ri > zero .and. Rrho <= zero) then
            out_pi(1) = pi0_1/(1.0d0 + Ri)
            out_pi(2) = pi0_2
            out_pi(3) = pi0_3
            out_pi(4) = pi0_4/(1.0d0 + Ri)
            out_pi(5) = pi0_5
        end if
    ...
    Subroutine dyn_time_scale_calc(Ri,Rrho,pi_n,Gm, cubic_coef,ii,jj)
        implicit none
    ...
    !    end if
        ...
        call shengjin_calc(cubic_coef,4,Gm,ii,jj)
    ...
    subroutine Thermocline_mixing_coeff_calc(Km_out, Kh_out, Ks_out, Kd_out,&
        ...
        real(r8), parameter :: f30 = 2.0d0*Omega*sin(30.0d0*degtorad)
        real(r8), parameter :: Tke_n2 = 0.288d-4
        
        real(r8) :: cond1, cond2
        
        f_lat = abs(2.0d0*Omega*sin(lat_in*degtorad)+very_small)
    ...
    subroutine canuto_2010_interface(Km_out,    Kh_out,     Ks_out,     Kd_out,     mld_out,&
                                     ...
                                     n2_in,     s2_in,      lat_in,     lev_in,     num_lev,ii,jj,iblock, isc)

        real(r8), parameter :: Rrho_bound = 1.d-3 
        real(r8), parameter :: Ri_low   = -1.d+10
        real(r8), parameter :: Ri_high   = 1.d+10
        integer, intent(in) :: num_lev, ii,jj, iblock, isc
        real(r8), intent(in) :: lev_in(km), s2_in(km-1), n2_in(km-1), lat_in
        real(r8), intent(in) :: ts_in(km), ss_in(km), rho_in(km), ri_in(km-1), rrho_in(km-1)
        real(r8), intent(out) :: Km_out(km), Kh_out(km), Ks_out(km), Kd_out(km), mld_out
        ...
        real(r8) :: Ri(km-1), Rrho(km-1), Gm(km-1), out_pi(5), Gm0
        real(r8) :: cube(4)
        integer :: mld_lev, k

        call find_mix_layer_depth(mld_out,mld_lev,rho_in,0.03_r8,lev_in,num_lev)
        ...
        do k = 1, num_lev - 1
            ...
            call dyn_time_scale_calc(1.0d10,Rrho(k),out_pi(:),Gm(k),cube,ii,jj)
            ...
            call dyn_time_scale_calc(Ri(k),Rrho(k),out_pi(:),Gm(k),cube,ii,jj)
!               if (mytid ==6 .and. i ==32 .and. j== 17 .and. isc ==41 .and. k == 5 ) then
!                   write(125,*) out_pi, ri(k), rrho(k), gm(k),cube
!               end if
...
!
                Km_out(k) = min(1.2D-1 , Km_out(k))
                Km_out(k) = max(0.1D-3 , Km_out(k))
                Kh_out(k) = min(1.2D-1 , Kh_out(k))
                Kh_out(k) = max(0.2D-4 , Kh_out(k))
                Ks_out(k) = min(1.2D-1 , Ks_out(k))
                Ks_out(k) = max(0.2D-4 , Ks_out(k))
!
!
!               if (mytid ==6 .and. i ==32 .and. j== 17 .and. isc ==41 ) then
!       print*,"!======================================================" , K, mld_lev
!       call check_single_value_real_r8("n2_in",n2_in(k))
!       call check_single_value_real_r8("s2_in",s2_in(k))
!       call check_single_value_real_r8("ri_in",ri_in(k))
!       call check_single_value_real_r8("rrho_in",rrho_in(k))
