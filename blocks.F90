...
!----------------------------------------------------------------------
   ...
   nblocks_x   = (nx_global-1)/licom_BlockSizeX + 1
   nblocks_y   = (ny_global-1)/licom_BlockSizeY + 1
...
!----------------------------------------------------------------------
   ...
   do jblock=1,nblocks_y
      ...
      if (js > ny_global) call exit_LICOM(sigAbort, &
      ...
      do iblock=1,nblocks_x
         ...
         if (is > nx_global) call exit_LICOM(sigAbort, &
...
!----------------------------------------------------------------------
   ...
   if (block_id < 1 .or. block_id > nblocks_tot) then
      call exit_LICOM(sigAbort,'get_block_parameter: invalid block_id')
   endif
