...
!-----------------------------------------------------------------------
   ...
   interface gather_global2
     module procedure gather_global2_dbl,  &
                      gather_global2_real
   end interface 
...
!***********************************************************************
!BOP
! !IROUTINE: gather_global2
! !INTERFACE:


subroutine gather_global2_dbl(ARRAY_G, ARRAY, dst_task, src_dist)
...
end subroutine gather_global2_dbl
!***********************************************************************
!***********************************************************************

 subroutine gather_global2_real(ARRAY_G, ARRAY, dst_task, src_dist)
...
!-----------------------------------------------------------------------
 ...
 end subroutine gather_global2_real
...
!-----------------------------------------------------------------------
   ...
   case (field_type_noupdate) ! ghost cells never used - use cell center
      isign =  1
   case default
      call exit_LICOM(sigAbort, 'Unknown field type in scatter')
   end select
               ...
               if (this_block%j_glob(j) > 0) then ! normal boundary
                  ...
                  do i=1,nx_block
                     if (this_block%i_glob(i) /= 0) then
                        ...
                        if (isrc < 1) isrc = isrc + imt_global
                        if (isrc > imt_global) isrc = isrc - imt_global
