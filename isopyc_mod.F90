...
!
      ...
      real(r8):: dptlim(nrpl+1), fzisop(km), slmxr
      real(r8),dimension(imt,jmt,max_blocks_clinic) :: ahisop, athkdf
      real(r8),dimension(:,:,:,:,:),allocatable:: e,rhoi
      real(r8),dimension(:,:,:,:,:),allocatable:: K1,K2,K3
      real(r8),dimension(:,:,:,:),allocatable:: adv_vetiso
      real(r8) :: adv_vbtiso(imt,0:km,jmt,max_blocks_clinic),adv_vntiso(imt,km,jmt,max_blocks_clinic)
#ifdef isopycmixspatialvar
...
!
      real(r8):: kisrpl(km) 
      integer:: krplin(nrpl)
      real(r8)::  zt(km),dzw(0:km),dzwr(0:km),dzr(km),tmask(imt,km,jmt,max_blocks_clinic)
      REAL(r8) , dimension(imt,jmt,max_blocks_clinic) :: F3
#endif
