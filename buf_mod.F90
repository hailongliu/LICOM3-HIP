...
! integer ,save, allocatable ::   mask_g(:,:), mask_r(:,:,:) ! domain mask: 0 <=> cell NOT in domain
  ...
  real(SHR_KIND_R8), allocatable ::   Qheat    (:,:,:)   ! heat of fusion    (q > 0) ~ W/m^2
  ...
  real(SHR_KIND_R8), allocatable ::   iceoff (:,:,:) ! water flux: iceoff        ~  kg/s/m^2
  real(SHR_KIND_R8), allocatable ::   snow1 (:,:,:)  ! water flux: snow1        ~  kg/s/m^2
